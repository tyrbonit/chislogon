(function() {
  ...
  var currency = function(summa, options) {
    ...
    if ('part10' in options.unit){
    	tnum[2] = tnum[1] ? common(tnum[1][1], options.unit.part, options.zero_part, options.off_part) : "";
      tnum[1] = tnum[1] ? common(tnum[1][0], options.unit.part10, options.zero_part, options.off_part) : "";
    } else {
    	tnum[1] = tnum[1] ? common(tnum[1], options.unit.part, options.zero_part, options.off_part) : "";
    };
    tnum = tnum.filter(function(x) {return x?true:false}).join(" ");
    return tnum.trim()
  };
